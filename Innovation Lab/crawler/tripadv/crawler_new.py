...
def parse_home_page(html):
    ...
    for item in items_from_page:
        item_dict = {}
        # name
        item_dict['name'] = item.find("div",
                                      attrs={"class":
                                             "airlineName"})['data-name']
        # reviews
        try:
            item_dict['reviews'] = int(
                item.find("div", attrs={
                    "class": "airlineReviews"
                }).contents[0].split(" ")[0].replace(',', ''))
...
def parse_detail_page(html):
    ...
    '''
    bsObj = BeautifulSoup(html, 'html.parser')
    reviews_from_page = bsObj.findAll(
        "div",
        attrs={
            "class":
    ...
    for review_item in reviews_from_page:
        reviews_dict = {}
        # title
        reviews_dict['title'] = review_item.find(
            "div",
            attrs={
                "class":
...
if __name__ == '__main__':
    ...
    print(Airlines)
    result.to_csv("./tripadv/home_page_info.csv")
